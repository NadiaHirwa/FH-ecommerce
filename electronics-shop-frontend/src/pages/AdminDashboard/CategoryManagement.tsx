...
export const CategoryManagement: React.FC = () => {
  ...
  const openEdit = (c: Category) => { setEditing(c); setForm({ name: c.name, slug: c.slug || '', image: c.image || ''}); setShowForm(true); };

  const save = async (e?: React.FormEvent) => {
    e?.preventDefault();
    const payload: Category = { id: editing ? editing.id : `c${Date.now()}`, name: form.name, slug: form.slug || undefined, active: editing ? editing.active : true, image: form.image || (editing ? editing.image : undefined) };
    if (editing) setItems(items.map(i => i.id === editing.id ? payload : i)); else setItems([payload, ...items]);
    setShowForm(false);
  };

  const remove = (id: string) => { if (!confirm('Delete category?')) return; setItems(items.filter(i => i.id !== id)); };
